...
func Sözlistele(HTMLcevap http.ResponseWriter, HTMListek *http.Request) {
	...
	ÖzlüSözler[4] = Söz{"Mert ÇALIŞKAN (this-Author)", 2021, `AT YALANI SİKİYİM İNANI`}

	//!JSON CEVABI OLUŞTUR
	liste, _ := json.MarshalIndent(ÖzlüSözler, " ", "")
	//!JSON ÖZELLİKLERİNİ AYARLA
	HTMLcevap.Header().Set("Content-Type", "application/json; charset=UTF-8")
	//!JSON CEVABINI GÖNDER
	HTMLcevap.Write(liste)
